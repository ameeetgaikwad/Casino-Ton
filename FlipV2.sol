...
contract Flip {
    ...
    event DetailedGameResult(address player, uint amountBet, uint8 guess, bool winner, uint totalPayout, int totalProfit);
   ...
   function flipit(uint8 guess) public payable returns(bool) {
    ...
    require(actualBet <= address(this).balance * 10 / 100, "You cannot bet more than what is available in the jackpot");
    ...
    emit DetailedGameResult(msg.sender, amountBet, guess, won, totalPayout, totalProfit);
    lastPlayedGames.push(Game(msg.sender, amountBet, guess, won, totalPayout, totalProfit));
    return won;
}
    ...
    function getGameEntry(uint index) public view returns(address addr, uint amountBet, uint8 guess, bool winner, uint totalPayout, int totalProfit) {
        return (
