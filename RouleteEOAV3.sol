...
contract Roulette {
    ...
    enum GuessType { Number, Even, Odd }
    ...
    event FinalResult(uint8 side, uint totalPayout, uint totalBetAmounts, bool isWon, int totalProfit);
    ...
    function processBet(uint8 guess, GuessType guessType, uint betAmount) private returns (bool, uint) {
    uint8 result = uint8(uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, now))) % 37);
    uint payout;
    ...
    if (guess == result && guessType == GuessType.Number) {
        ...
        return (true, payout);
    } else if (guess % 2 == 0 && guessType == GuessType.Even && result % 2 == 0) {
        // Correct guess for even, x2 payout
        payout = betAmount * 188 / 100;
        return (true, payout);
    } else if (guess % 2 != 0 && guessType == GuessType.Odd && result % 2 != 0) {
    ...
    function roulette(uint8[] memory guesses, GuessType[] memory guessTypes, uint[] memory betAmounts) public payable returns(bool) {
    require(guesses.length == guessTypes.length && guessTypes.length == betAmounts.length, "Mismatch in array lengths");
    require(msg.value >= getTotalBetAmount(betAmounts), "Insufficient bet amount");
    ...
    if (isContract) {
        ...
        for (uint i = 0; i < guesses.length; i++) {
            if (guesses[i] == uint8(uint256(keccak256(abi.encodePacked(block.difficulty, msg.sender, block.timestamp))) % 2)) {
    ...
    lastPlayedGames.push(Game(msg.sender, getTotalBetAmount(betAmounts), guesses[i], won, address(this).balance, guessTypes[i], payout));
}

// Emit the results for all guess types chosen
for (uint j = 0; j < guessTypes.length; j++) {
    emit GameResult(result, guessTypes[j], lastPlayedGames[lastPlayedGames.length - guessTypes.length + j].winner, lastPlayedGames[lastPlayedGames.length - guessTypes.length + j].payout);
}

// Emit final result with playerWonAnyBet determining if the player won any bet
emit FinalResult(result, totalPayout, getTotalBetAmount(betAmounts), playerWonAnyBet, int(totalPayout) - int(getTotalBetAmount(betAmounts)));
    ...
    function getTotalBetAmount(uint[] memory betAmounts) private pure returns (uint) {
        uint totalBetAmount;
